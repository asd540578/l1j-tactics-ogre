...
public class CalcExp {
	...
	/* PvP時の経験値取得 */
	public static void calcExp(L1PcInstance lastAttacker, L1PcInstance endPc) {
		int exp4next = ExpTable.getNeedExpNextLevel(lastAttacker.getLevel());
		int atLv = lastAttacker.getLevel(), endLv = endPc.getLevel();
		if (endLv >= atLv) // 死亡者 >= 攻撃者
		{
			lastAttacker.addExp((int) (exp4next * 0.02)); // ２％の経験値を取得する
		}
	}
	...
	public static double calcExpBonusRate(L1PcInstance pc) {
		...
		double bonus = pc.getExpBonusPct() / 100.0 + 1.0;
	...
	private static void addExpAndLawful(L1PcInstance pc, int exp, int lawful) {
		...
		pc.addExp((int) (exp * calcExpBonusRate(pc)));
	}
