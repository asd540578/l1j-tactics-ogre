		...
		if (pc.getCurrentHp() > 0)
		{
			...
			{ // 風の武器強化スクロール～火の武器強化スクロール
				...
				else
				{
					pc.sendPackets(new S_ServerMessage(1411, l1iteminstance1
							.getLogName(), "$245", "$247")); // f1%0に魔法が入り込めません。
				}
				pc.getInventory().removeItem(l1iteminstance, 1);
			}
			else if (itemId == 49304 || itemId == 49305 || itemId == 49306
					|| itemId == 49307 || itemId == 49308 || itemId == 49309)
			{ // マリスエレメント武器(etcitem)
				int weapon_id = 0;
				int newAttrEnchantKind[] =
				{
						1, 2, 4, 8
				};
				...
				{
				case 49304:
					weapon_id = 276; // マリス エレメント キーリンク
					break;
				case 49305:
					weapon_id = 277; // マリス エレメント チェーンソード
					break;
				case 49306:
					weapon_id = 278; // マリス エレメント クロウ
					break;
				case 49307:
					weapon_id = 279; // マリス エレメント スタッフ
					break;
				case 49308:
					weapon_id = 280; // マリス エレメント ボウ
					break;
				case 49309:
					weapon_id = 281; // マリス エレメント ソード
					break;
				}
				pc.getInventory().removeItem(l1iteminstance, 1);
				...
				// 属性の付与
				item.setAttrEnchantKind(newAttrEnchantKind[rnd_attr]);
			...
			{ // 種別：その他のアイテム
				...
				{ // treasure_box
					...
					if (itemId == 50499)
					{// デスマッチ記念品の箱
						if (// pc.getMap().getBaseMapId()!=5153){
						pc.getMapId() != 5153)
						{
							return;
						}
					...
					if (itemId == 50500)
					{ // ドルーガバッグ
						if (l1iteminstance.getChargeCount() >= 1)
						{
				...
				{ // 祝福されたウィズダム
					...
					{
						pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでし。
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				else if (itemId == 50501)
				{ // ドラゴンキー(レイド選択コード不明)
					L1SpawnUtil.spawn(pc, 91051, 0, 7200000); // 120*60*1000 =
					// 2時間
					pc.getInventory().removeItem(l1iteminstance, 1);
				...
				{ // 糊
					...
					if (diaryId >= 41038 && 41047 >= diaryId)
					{
						...
						{
							L1ItemUtil.createNewItem(pc, diaryId + 10, 1);
						}
						else
						{
				...
				else if (itemId >= 41048 && 41055 >= itemId)
				{
					...
					{
						L1ItemUtil.createNewItem(pc, logbookId + 2, 1);
						pc.getInventory().removeItem(l1iteminstance1, 1);
						pc.getInventory().removeItem(l1iteminstance, 1);
					}
					else
				...
				else if (itemId == 41056 || itemId == 41057)
				{
					...
					if (logbookId == (itemId + 8034))
					{
						L1ItemUtil.createNewItem(pc, 41058, 1);
						pc.getInventory().removeItem(l1iteminstance1, 1);
						pc.getInventory().removeItem(l1iteminstance, 1);
					}
					else
				...
				{ // 浄化のポーション
					...
					{ // 呪われたブラックイアリング
						...
						{
							L1ItemUtil.createNewItem(pc, earingId + 186, 1);
						}
						else
						{
							pc.sendPackets(new S_ServerMessage(158,
				...
				else if (itemId >= 40926 && 40929 >= itemId)
				{
					...
					if (earing2Id >= 41173 && 41184 >= earing2Id)
					{
								...
								&& (itemId + potion2) >= earing2Id)
						{
							...
							{
								L1ItemUtil.createNewItem(pc, (earing2Id - 12),
										1);
								pc.getInventory()
					...
					if (earing3Id >= 41161 && 41172 >= earing3Id)
					{
						...
						if (earing3Id == (itemId + 230))
						{
							if ((_random.nextInt(99) + 1) < Config.CREATE_CHANCE_PROCESSING)
							{
								...
								L1ItemUtil.createNewItem(pc, earinglevel, 1);
							}
							else
							{
								pc.sendPackets(new S_ServerMessage(158,
						...
						if (ringId == (itemId + 242))
						{
							if ((_random.nextInt(99) + 1) < Config.CREATE_CHANCE_PROCESSING_DIAMOND)
							{
								...
								pc.sendPackets(new S_ServerMessage(gmas,
										l1iteminstance1.getName()));
								L1ItemUtil.createNewItem(pc, ringlevel, 1);
								pc.getInventory()
										.removeItem(l1iteminstance1, 1);
								pc.getInventory().removeItem(l1iteminstance, 1);
							}
				...
				{ // 召喚球の欠片
					...
					{ // 召喚球のコア・各段階
						...
						{
							L1ItemUtil.createNewItem(pc, dantesId + 1, 1);
						}
						else
						{
							pc.sendPackets(new S_ServerMessage(158,
				...
				{ // ダークマジックパウダー
					...
					if (historybookId >= 41011 && 41018 >= historybookId)
					{
						...
						{
							L1ItemUtil.createNewItem(pc, historybookId + 8, 1);
						}
						else
						{
							pc.sendPackets(new S_ServerMessage(158,
					...
					{ // ウィザード
						...
						{ // ブランク
							...
							if (spellsc != null)
							{
								...
								{
									L1Skill l1skills = SkillTable.getInstance()
											.findBySkillId(blanksc_skillid + 1); // blanksc_skillidは0始まり
				...
				{ // 40863はテレポートスクロールとして処理される
					if (spellsc_objid == pc.getId()
							&& l1iteminstance.getItem().getUseType() != 30)
					{ // spell_buff
				...
				{ // パインワンド
					if (pc.getMap().isUsePainwand())
					{
						...
						int[] mobArray =
						{
								45008, 45140, 45016, 45021, 45025, 45033,
								45099, 45147, 45123, 45130, 45046, 45092,
								45138, 45098, 45127, 45143, 45149, 45171,
								45040, 45155, 45192, 45173, 45213, 45079, 45144
						};
						...
						if (itemId == 40006 || itemId == 140006)
						{
							...
							{
								pc.getInventory().deleteItem(l1iteminstance);
							}
				...
				else if (itemId >= 40280 && itemId <= 40288)
				{
					...
					// 肉類
						...
						|| itemId == 40060 || itemId == 40061
						|| itemId == 40062 || itemId == 40063
						|| itemId == 40064 || itemId == 40065
						|| itemId == 40069 || itemId == 40072
						|| itemId == 40073 || itemId == 140061
						|| itemId == 140062 || itemId == 140065
						|| itemId == 140069 || itemId == 140072
						|| itemId == 41296 || itemId == 41297
						|| itemId == 41266 || itemId == 41267
						|| itemId == 41274 || itemId == 41275
						|| itemId == 41276 || itemId == 41252
						|| itemId == 49040 || itemId == 49041
						|| itemId == 49042 || itemId == 49043
...
//					pc.getInventory().removeItem(l1iteminstance, 1);
				...
				{ // シャイニングレッドフィッシュ
					...
					{
						L1ItemUtil.createNewItem(pc, 40019, 1);
					}
					else if (chance >= 9)
					{
						...
						if (gemChance == 0)
						{
							L1ItemUtil.createNewItem(pc, 40045, 1);
						}
						else if (gemChance == 1)
						{
							L1ItemUtil.createNewItem(pc, 40049, 1);
						}
						else if (gemChance == 2)
						{
							L1ItemUtil.createNewItem(pc, 40053, 1);
						}
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				{ // シャイニンググリーンフィッシュ
					...
					{
						L1ItemUtil.createNewItem(pc, 40018, 1);
					}
					else if (chance >= 9)
					{
						int gemChance = _random.nextInt(3);
						if (gemChance == 0)
						{
							L1ItemUtil.createNewItem(pc, 40047, 1);
						}
						else if (gemChance == 1)
						{
							L1ItemUtil.createNewItem(pc, 40051, 1);
						}
						else if (gemChance == 2)
						{
							L1ItemUtil.createNewItem(pc, 40055, 1);
						}
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				{ // シャイニングブルーフィッシュ
					...
					{
						L1ItemUtil.createNewItem(pc, 40015, 1);
					}
					else if (chance >= 9)
					{
						int gemChance = _random.nextInt(3);
						if (gemChance == 0)
						{
							L1ItemUtil.createNewItem(pc, 40046, 1);
						}
						else if (gemChance == 1)
						{
							L1ItemUtil.createNewItem(pc, 40050, 1);
						}
						else if (gemChance == 2)
						{
							L1ItemUtil.createNewItem(pc, 40054, 1);
						}
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				{ // シャイニングホワイトフィッシュ
					...
					{
						L1ItemUtil.createNewItem(pc, 40021, 1);
					}
					else if (chance >= 9)
					{
						int gemChance = _random.nextInt(3);
						if (gemChance == 0)
						{
							L1ItemUtil.createNewItem(pc, 40044, 1);
						}
						else if (gemChance == 1)
						{
							L1ItemUtil.createNewItem(pc, 40048, 1);
						}
						else if (gemChance == 2)
						{
							L1ItemUtil.createNewItem(pc, 40052, 1);
						}
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				{ // カヘルの契約書
					...
					{
						L1ItemUtil.createNewItem(pc, 41122, 1);
					}
				}
				...
				{ // 血痕の契約書
					...
					{
						L1ItemUtil.createNewItem(pc, 41131, 1);
					}
				}
				...
				{ // 歪みのコア
					...
					{ // 閉ざされたオシリスの宝箱
						L1ItemUtil.createNewItem(pc, targetItemId + 1, 1);
						pc.getInventory().consumeItem(targetItemId, 1);
						pc.getInventory().consumeItem(49092, 1);
					}
					else
					{
						if (targetItemId == 49312)
						{ // 閉ざされた上級ククルカンの宝箱
							L1ItemUtil.createNewItem(pc, 49311, 1);
							pc.getInventory().consumeItem(targetItemId, 1);
							pc.getInventory().consumeItem(49092, 1);
						}
						else
						{
							if (targetItemId == 49313)
							{ // 閉ざされた下級ククルカンの宝箱
								L1ItemUtil.createNewItem(pc, 49310, 1);
								pc.getInventory().consumeItem(targetItemId, 1);
								pc.getInventory().consumeItem(49092, 1);
							}
							else
				...
				{ // 下級オシリスの宝箱の欠片：上
					if (pc.getInventory().checkItem(49094, 1))
					{
						...
						L1ItemUtil.createNewItem(pc, 49095, 1);
					}
					else
					{
						pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでした。
				...
				{ // 上級オシリスの宝箱の欠片：上
					if (pc.getInventory().checkItem(49098, 1))
					{
						...
						L1ItemUtil.createNewItem(pc, 49099, 1);
					}
					else
					{
						pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでした。
				...
				{ // 封印スクロール
							...
							&& lockItem.getItem().isCanSeal())
					{
						...
						{
							pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでしたF。
						}
					}
					else
					{
				...
				{ // 太古の玉爾
					...
					{
						L1Account account = L1Account.findByName(pc
								.getAccountName());
						if (account == null)
						{
							pc.sendPackets(new S_ServerMessage(79)); // \
				...
				{ // 下級ククルカンの宝箱の欠片：上
					if (pc.getInventory().checkItem(49317, 1))
					{
						pc.getInventory().consumeItem(49317, 1);
						pc.getInventory().consumeItem(49316, 1);
						L1ItemUtil.createNewItem(pc, 49313, 1);
					}
					else
					{
						pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでした
				...
				{ // 下級ククルカンの宝箱の欠片：下
					if (pc.getInventory().checkItem(49316, 1))
					{
						pc.getInventory().consumeItem(49316, 1);
						pc.getInventory().consumeItem(49317, 1);
						L1ItemUtil.createNewItem(pc, 49313, 1);
				...
				{ // 上級ククルカンの宝箱の欠片：上
					if (pc.getInventory().checkItem(49315, 1))
					{
						pc.getInventory().consumeItem(49314, 1);
						pc.getInventory().consumeItem(49315, 1);
						L1ItemUtil.createNewItem(pc, 49312, 1);
					}
					else
					{
						pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでした
				...
				{ // 上級ククルカンの宝箱の欠片：下
					if (pc.getInventory().checkItem(49314, 1))
					{
						pc.getInventory().consumeItem(49315, 1);
						pc.getInventory().consumeItem(49314, 1);
						L1ItemUtil.createNewItem(pc, 49312, 1);
			...
			ClientThread client, int i)
	{
		...
		if (item.getItem().getType2() == 1)
		{
			...
			else
			{
				...
				case -1:
					s = (new StringBuilder())
							.append(pm + item.getEnchantLevel()).append(" ")
	...
	private void UseHeallingPotion(L1PcInstance pc, int healHp, int gfxid)
	{
		...
		healHp *= (_randomForGaussian.nextGaussian() / 5.0D) + 1.0D;
	...
	private void UseArmor(L1PcInstance activeChar, L1ItemInstance armor)
	{
		...
		{ // 使用した防具を装備していなくて、その装備箇所が空いている場合（装着を試みる）
			...
			{// シールドの場合、武器を装備していたら両手武器チェック
				...
				{// 両手武器判定
					if (activeChar.getWeapon().getItem().getItemId() == 277)
					{// エレメンタルチェーンソードの場合両手武器として判定しない
					}
					else
					{
	...
	private void UseWeapon(L1PcInstance activeChar, L1ItemInstance weapon)
	{
		...
		{ // 指定された武器が装備している武器と違う場合、装備できるか確認
			...
			{// 両手武器の場合、シールド装備の確認
				if (weapon.getItem().getItemId() == 277)
				{// エレメンタルチェーンソードの場合両手武器として判定しない
				}
				else
				{
	...
	private void polyAction(L1PcInstance attacker, L1Character cha)
	{
		...
		else if (cha instanceof L1MonsterInstance)
		{
			...
			if (mob.getLevel() < 50)
			{
				...
				if (npcId != 45338 && npcId != 45370 && npcId != 45456 // クロコダイル、
						// バンディットボス
						// 、
						// ネクロマンサー
						&& npcId != 45464 && npcId != 45473 && npcId != 45488 // セマ
						// 、
						// バルタザール
						// 、
						// カスパー
						&& npcId != 45497 && npcId != 45516 && npcId != 45529 // メルキオール
						// 、
						// イフリート
						// 、
						// ドレイク
	...
	{ // アブソルート バリアの解除
		if (pc.hasSkillEffect(ABSOLUTE_BARRIER))
		{
			...
			pc.startHpRegenerationByDoll(); // TODO マジックドール HPR開始
			...
			L1ItemInstance resolvent)
	{
		...
		{
			crystalCount = 0;
			pc.sendPackets(new S_ServerMessage(158, item.getName())); // \f1%0
			// が蒸発してなくなりました
			// 。
		}
