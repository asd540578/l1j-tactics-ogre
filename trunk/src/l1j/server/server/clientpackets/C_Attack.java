...
public class C_Attack extends ClientBasePacket {
	...
	public C_Attack(byte[] decrypt, ClientThread client) {
		...
		/* t.s add start 2011/08/22 */
		// 弓でRange以上の距離の場合、攻撃を中止する
		if (null != pc.getWeapon()) {
			boolean isBow = (20 == pc.getWeapon().getItem().getType1());
			if (isBow) {
				int range = 0x7fffffff & pc.getWeapon().getItem().getRange();
				boolean isMoreLongRange = (pc.getLocation()
						.getTileLineDistance(new Point(_targetX, _targetY)) > range);
				if (isMoreLongRange) {
					return;
				}
			}
		}
		/* t.s add end 2011/08/22 */
