...
public class L1MonsterInstance extends L1NpcInstance
{
	...
	public void onAction(L1PcInstance player, int skillId)
	{
		...
		if (attack.calcHit())
		{
			if (hasSkillEffect(COUNTER_BARRIER))
			{
				L1Magic magic = new L1Magic(this, player);
				boolean isProbability = magic
						.calcProbabilityMagic(COUNTER_BARRIER);
				boolean isShortDistance = attack.isShortDistance();
				if (isProbability && isShortDistance)
		...
		/*
		 * if (getCurrentHp() > 0 && !isDead()) { L1Attack attack = new
		 * L1Attack(player, this, skillId); if (attack.calcHit()) {
		 * attack.calcDamage(); attack.calcStaffOfMana();
		 * attack.addPcPoisonAttack(player, this); attack.addChaserAttack(); }
		 * attack.action(); attack.commit(); }
		 */
	}
	...
	{
		if (Config.BOSS_END_LOG == false)
		{ // boss_end_log on/off判定
		}
		else
		{
			...
			if (boss == true)
			{
				...
				String bossname = getNpcTemplate().get_name();
