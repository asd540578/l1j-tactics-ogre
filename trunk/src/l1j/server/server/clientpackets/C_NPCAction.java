...
public class C_NPCAction extends ClientBasePacket {
	...
	public C_NPCAction(byte abyte0[], ClientThread client) throws Exception {
		...
		if (s.equalsIgnoreCase("buy")) {
			...
			// TODO アライメント回復
		} else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 80166) { // 善の使者
			// 　
			// ユリウス
			...
			if (npc.getNpcTemplate().get_npcId() == 60514) { // ケント城近衛兵
				...
				htmlid = "wdguard7";
			} else if (npc.getNpcTemplate().get_npcId() == 60524 || // ギラン街入り口近衛兵(
					// 弓)
				...
				htmlid = "adguard7";
			} else if (npc.getNpcTemplate().get_npcId() == 81156) { // アデン偵察兵（
				// ディアド要塞）
		...
		else if (s.equalsIgnoreCase("fire")) // エルフの属性変更「火の系列を習う」
		{
		...
		} else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 80167) { // 善の使者
			// 　
			// ミリエル
			...
			if (npcId == 80085) { // お化け屋敷の管理人　デュオ
				...
				l1j.server.server.model.L1DeathMatch.getInstance()
						.enterGame(pc);
			} else if (npcId == 80087) { // 　デスマッチ管理人　ダトゥ
				// l1j.server.server.model.L1DeathMatchExpert.getInstance().
				// enterGame(pc);
		...
		// エルラス
			...
			if (s.equalsIgnoreCase("0")) {
				...
				|| ((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 70042) { // ギランレース管理人
			// 　
			// セシル
			// 　
			// ポーリー
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 91327) {
			if (s.equalsIgnoreCase("c")) {
					...
					if (item != null) {
						pc.sendPackets(new S_ServerMessage(143,
								((L1NpcInstance) obj).getNpcTemplate()
										.get_name(), item.getItem().getName()));
						// \f1%0が%1をくれました。
						pc.getQuest().set_step(L1Quest.QUEST_YURIE,
								L1Quest.QUEST_END);
					}
				}
				htmlid = "";
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 80192) {
		...
		// ペット商人・引き換え
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 70077 // ロドニー
				|| ((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 91056) { // バンク
			SellOfPet(s, pc);
			htmlid = "";
