			...
			L1Character attacker)
	{
		...
		try
		{
			...
			{ // 魔法詠唱時
				...
				{
					// _log.severe(_skillId + ":" + _skill.getCastGfx()
					// + " : runSkill() before call");
					runSkill();
					useConsume();
					// Thread.sleep(2000);
					// _log.severe(_skillId + ":" + _skill.getCastGfx()
					// + " : sendGrfx() before call");
					sendGrfx(true);
					// Thread.sleep(2000);
...
/*   t.s 2012/2/5 mod end   */
			...
			{ // 補助魔法
			  // テレポート、マステレ、テレポートトゥマザー以外
				if (_skillId != 5 && _skillId != 69 && _skillId != 131)
				{
					...
					/* t.s 2011/12/31 mod start */
					/* CBは相手プレイヤーにも見えるよう修正 */
					// if (_skillId == COUNTER_MAGIC || _skillId ==
					// COUNTER_MIRROR || _skillId == COUNTER_BARRIER) {
	...
	private void deleteRepeatedSkills(L1Character cha)
	{
		final int[][] repeatedSkills =
		{
				...
				{
						SHIELD, SHADOW_ARMOR, EARTH_SKIN, EARTH_BLESS,
						IRON_SKIN
				},
				// ホーリー ウォーク、ムービング アクセレーション、ウィンド ウォーク、BP、ワッフル、ブラッドラスト
				{
						HOLY_WALK, MOVING_ACCELERATION, WIND_WALK,
						STATUS_BRAVE, STATUS_ELFBRAVE, BLOODLUST
				},
				// ヘイスト、グレーター ヘイスト、GP
				{
...
// t.s 2012/02/12 mod end
				...
				{ // トリプルアロー
				  ...
				  // その後はアローの射出を行わず動きだけを行う。
					...
					int[] BowGFX =
					{
							138, 37, 3860, 3126, 3420, 2284, 3105, 3145, 3148,
							3151, 3871, 4125, 2323, 3892, 3895, 3898, 3901,
					...
					if (_user instanceof L1PcInstance)
					{
						...
						_player.sendPackets(new S_SkillSound(_player.getId(),
								4394));
						_player.broadcastPacket(new S_SkillSound(_player
								.getId(), 4394));
					}
					else if (_user instanceof L1NpcInstance)
					{
						int npcGFX = _user.getTempCharGfx();
						for (int gfx : BowGFX)
						{
							if (npcGFX == gfx)
							{
				...
				{ // フォースレイヤー
					...
					if (_user instanceof L1PcInstance)
					{
						...
						_player.sendPackets(new S_SkillSound(_player.getId(),
								7020));
						_player.broadcastPacket(new S_SkillSound(_target
								.getId(), 6509));
						_player.broadcastPacket(new S_SkillSound(_player
								.getId(), 7020));

						if (_player
								.hasSkillEffect(STATUS_WEAKNESS_EXPOSURE_LV1))
						{
