...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance pc) {
		...
		if (talking != null) {
			if (npcid == 70841) { // ルーディエル
				...
				if (pc.isDarkelf()) {
					if (pc.getTempCharGfx() == 3634) { // アサシン変身
						int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
						if (lv45_step == 1) {
							htmlid = "assassin1";
						} else if (lv45_step == 2) {
				...
				if (pc.getQuest().get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END) {
					...
					htmlid = "minicod05";
				} else if (pc.getQuest().get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END // 影の神殿側クエスト終了
				...
				if (pc.getQuest().get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END) {
					...
					htmlid = "minitos05";
				} else if (pc.getQuest().get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END // 欲望の洞窟側クエスト終了
				...
				/*
				 * // イリュージョニスト　クエストレベル４５　　未調整 } else if (npcid == 81247) { //
				 * ホワイトアントの死体(1F) if(pc.isIllusionist()) { int lv30_step =
				 ...
				 * (pc.isDragonKnight()) { // ドラゴンナイト if(pc.getLevel() >= 45 &&
				 ...
				 * (pc.isIllusionist()) { // イリュージョニスト if(pc.getLevel() > 49 &&
				...
				/*
				 * 初心者クエスト　未調整 } else if (npcid == 80171) { // 初心者案内人 int
				 ...
				 * pc.addExp(125); // exp付与　lv2 L1ItemInstance item =
				 ...
				 * 修練所管理人 int quest_step = quest.get_step(L1Quest.QUEST_TUTOR);
	...
	/*
	 * 初心者クエスト　未調整 private void newUserHelp(L1PcInstance pc, int helpNo) {
	 * switch (helpNo) { case 1:// ヘイスト & HP/MP全回復 pc.sendPackets(new
