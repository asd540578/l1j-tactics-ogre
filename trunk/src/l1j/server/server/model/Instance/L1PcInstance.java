...
import l1j.server.server.model.inventory.L1WarehouseInventory;
...
public class L1PcInstance extends L1Character {
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			deathPenalty(lastAttacker); // EXPロスト
			if (lastAttacker instanceof L1PcInstance) // PvPでEndさせた場合経験値取得
			{
				CalcExp.calcExp((L1PcInstance) lastAttacker, L1PcInstance.this);
			}
			...
			/* t.s 2011/09/19 mod start */
			/* PKログの出力位置変更（アイテムドロップの前） */
			// TODO PK情報 start
			L1PcInstance player = null;
			if (Config.PK_LOG == false) { // pklog on/off判定
				...
				if (lastAttacker instanceof L1PcInstance) {
					...
					String deadName = getName();
					String killerClan = (player.getClanid() == 0) ? "未所属"
							: player.getClanname();
					String deadClan = (getClanid() == 0) ? "未所属"
							: getClanname();
					int killerLevel = player.getLevel();
					int deadLevel = getLevel();
					int mapid = player.getMapId();
					String areaname = player.getMap().locationname();

					// 履歴作成用ＰＫログ
					L1PkLog pkLog = new L1PkLog(killerName, killerClan,
							killerLevel, deadName, deadClan, deadLevel, mapid,
							areaname);
					// ＤＢに登録する
					pkLog.storePkHistory();

					world.broadcastServerMessage(pkLog.buildKillMessage());
					world.broadcastServerMessage(pkLog.buildDeathMessage());
					world.broadcastServerMessage("Location:" + areaname);
				}
			}
			// TODO PK情報 end
			/* t.s 2011/09/19 mod end */
			...
			if (lostRate < 0) {
				...
				if (rnd <= lostRate) {
					...
					/* t.s 2011/09/19 mod start */
					// アイテムロスト時に、相手キャラへ渡す処理を追加する
					caoPenaltyResult(lastAttacker, count);
					/* t.s 2011/09/19 mod end */
				}
			}
			...
			if (player != null) {
				if (getLawful() >= 0 && isPinkName() == false) {
					...
					if (Config.HELL == true) {
						...
						if (isChangePkCount
								&& player.get_PKcount() >= PkCountLimitMin
								&& player.get_PKcount() < PkCountLimitMax) {
							// あなたのPK回数が%0になりました。回数が%1になると地獄行きです。
							player.sendPackets(new S_BlueMessage(551, String
									.valueOf(player.get_PKcount()), String
	...
	private void caoPenaltyResult(L1Character lastAttacker, int count) {
		for (int i = 0; i < count; i++) {
			...
			if (item != null) {
				// ロストしたアイテム情報
				int lostCount = item.isStackable() ? _random.nextInt(item
						.getCount() + 1) : 1;
				String itemName = item.isStackable() ? item
						.getLogName(lostCount) : item.getLogName();
				/* t.s 2011/09/19 add start */
				// 最終攻撃者がプレイヤーの場合、直接インベントリーへ渡す処理を追加
				if (null != lastAttacker
						&& lastAttacker instanceof L1PcInstance) {
					L1PcInstance lastPc = (L1PcInstance) lastAttacker;
					...
					if (null == this.getParty()) {
						...
						for (L1PcInstance member : lastPc.getParty()
								.getMembers()) {
							member.sendPackets(lostPartyMsg);
						}
					}
					// 取得者（単騎）へのメッセージ
	...
	public void deathPenalty(L1Character lastAttacker) {
		int oldLevel = getLevel();
		int needExp = ExpTable.getNeedExpNextLevel(oldLevel);
		int exp = 0;
		...
		if (oldLevel >= 1 && oldLevel < 11) {
			...
			if (lastAttacker instanceof L1PcInstance) { // PKされた場合5%ロスト
				exp = (int) (needExp * 0.05);
			} else {
				exp = (int) (needExp * 0.025); // モンスターエンドの場合2.5%ロスト
				// exp = (int) (needExp * 0.05);
			}
		}
		/* t.s 2011/09/19 mod end */
